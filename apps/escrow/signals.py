...
from apps.core.choices import MissionStatus, EscrowStatus

@receiver(post_save, sender=Mission)
def handle_mission_escrow(sender, instance, created, **kwargs):
    # Si la mission vient d'être acceptée par un agent
    if instance.status == MissionStatus.ACCEPTED and not hasattr(instance, 'escrow'):
        EscrowService.lock_funds(instance)
    
    # Si la mission est terminée, on libère les fonds
    elif instance.status == MissionStatus.COMPLETED and hasattr(instance, 'escrow'):
        if instance.escrow.status == EscrowStatus.HELD:
